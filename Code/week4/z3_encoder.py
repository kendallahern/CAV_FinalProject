...
def verify_point(model_json_path, x0, eps, delta=0.0, timeout=None):
    ...
    if res == sat:
        m = solver.model()
        # collect input counterexample (flattened, length 28*28)
        ce = []
        for i in range(28):
            for j in range(28):
                try:
                    ...
                    # If Z3 named input differently for some reason, try fallback:
                    raise RuntimeError(f"Z3 model missing input variable x_0_{i}_{j}")
                # convert to float robustly
                try:
                    # use helper to handle fraction/decimal "?" cases
                    fval = rational_from_z3(v)
                except Exception:
                    # As last resort, string-parsing
                    s = v.as_decimal(20)
                    if "?" in s: s = s.replace("?", "")
                    fval = float(s)
                ce.append(float(fval))
        # sanity check: ensure length
        if len(ce) != 28*28:
            raise RuntimeError(f"counterexample length unexpected: {len(ce)}")
