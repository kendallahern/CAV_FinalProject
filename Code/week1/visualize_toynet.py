...
def plot_decision_boundary(model, X, y, test_point=None, eps=0.05, adv_point=None, graph_caption=''):
    """
    ...
    x_coord = x_min + (x_max - x_min) * 0.01 
    y_coord = y_max - (y_max - y_min) * 0.01
    text_string = f'{graph_caption} ε = {eps}'
    plt.text(x_coord, y_coord, text_string, verticalalignment='top')
    plt.legend()
    plt.show()
